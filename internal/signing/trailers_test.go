...
func TestClaimIsRefusedUnlessAllThreeTrailersAgreeWithTheSPIFFEID(t *testing.T) {
	cases := []struct {
		...
		{"lowercase task_ref", Claim{Identity: refClaim.Identity, Run: "run-42", Task: "jira-118"}, nil},
		{"empty identity", Claim{Identity: "", Run: "run-42", Task: "JIRA-118"}, ErrClaim},
		{"identity is not a SPIFFE ID", Claim{Identity: "innsegl.dev/agent/a/b/c", Run: "run-42", Task: "JIRA-118"}, ErrClaim},
		{"identity outside the /agent/ subtree",
			Claim{Identity: "spiffe://innsegl.dev/workload/fix-ci/jira-118/run-42", Run: "run-42", Task: "JIRA-118"}, ErrClaim},
		{"identity with too few segments",
			Claim{Identity: "spiffe://innsegl.dev/agent/fix-ci/run-42", Run: "run-42", Task: "JIRA-118"}, ErrClaim},
		{"identity segment outside the grammar",
			Claim{Identity: "spiffe://innsegl.dev/agent/fix_ci/jira-118/run-42", Run: "run-42", Task: "JIRA-118"}, ErrClaim},
		{"run does not match the identity's run segment",
			Claim{Identity: refClaim.Identity, Run: "run-43", Task: "JIRA-118"}, ErrClaim},
		{"run is empty", Claim{Identity: refClaim.Identity, Run: "", Task: "JIRA-118"}, ErrClaim},
		{"task does not lowercase to the identity's task segment",
			Claim{Identity: refClaim.Identity, Run: "run-42", Task: "JIRA-119"}, ErrClaim},
		{"task is empty", Claim{Identity: refClaim.Identity, Run: "run-42", Task: ""}, ErrClaim},
		{"task carries a newline", Claim{Identity: refClaim.Identity, Run: "run-42", Task: "JIRA-118\nAgent-Run: forged"}, ErrClaim},
		{"identity carries a newline",
			Claim{Identity: "spiffe://innsegl.dev/agent/fix-ci/jira-118/run-42\nAgent-Run: forged", Run: "run-42", Task: "JIRA-118"}, ErrClaim},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
